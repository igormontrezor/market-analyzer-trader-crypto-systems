...
def plot_institucional_chart():
    ...
    try:
        ...
        # 4. Criar figura com 5 subplots (USDT.D+indicadores, BB%B, Estocástico, MACD, Funding)
        fig = make_subplots(
            rows=5, cols=1,
            shared_xaxes=True,
            vertical_spacing=0.02,
            row_heights=[0.38, 0.14, 0.14, 0.14, 0.20],
            subplot_titles=[
        ...
        fig.add_hline(y=0.08, line_dash="dash", line_color="red", opacity=0.8, row=5, col=1)
        ...
        fig.update_yaxes(title_text="Funding Rate (%)", row=5, col=1, range=[-0.02, 0.10])
        ...
        # ── GRÁFICO OTHERS ────────────────────────────────────────────────────
        st.markdown("---")
        st.markdown("### 📊 OTHERS — Market Cap Excl. Top 10")
        try:
            ...
            if others_weekly is None or others_weekly.empty:
                st.warning("❌ Não foi possível obter dados do OTHERS")
            else:
                ...
                fig_o.add_trace(go.Bar(
                    ...
                    marker_color=_o_colors, name="MACD Hist OTHERS", opacity=0.7, showlegend=False
                ), row=3, col=1)
                fig_o.add_trace(go.Scatter(
                ...
                fig_o.add_trace(go.Scatter(
                    ...
                    name="Signal OTHERS", line=dict(color="#e3b341", width=1.2, dash="dot")
                ), row=3, col=1)
                fig_o.add_hline(y=0, line_dash="dot", line_color="rgba(200,200,200,0.25)", row=3, col=1)
                ...
                fig_o.update_yaxes(title_text="MACD",         row=3, col=1)
                ...
                with st.expander("📊 Análise OTHERS"):
                    ...
                    macd_val = float(_o_macd.iloc[-1])
                    sig_val  = float(_o_signal.iloc[-1])
                    co3.metric("MACD vs Signal", f"{macd_val:.2f}",
                               "Alta" if macd_val > sig_val else "Baixa")

        except Exception as e_o:
            st.warning(f"❌ Erro ao obter dados do OTHERS: {e_o}")
